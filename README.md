# Amazon Financial Dashboard

## Project Overview

This project analyzes Amazon's financial performance using Microsoft Excel.

The dashboard includes financial KPIs, trend analysis, and ratio analysis from 2015–2024.

## Tools Used

- Microsoft Excel
- Financial Statement Analysis
- Dashboard Design
- Financial Ratios

## KPIs

- Revenue
- Net Income
- ROA
- Current Ratio
- Debt-to-Equity
- Revenue Growth
- Asset Turnover

## Key Insights

- Revenue grew consistently.
- Profitability fluctuated during some years.
- Liquidity remained healthy.
- Asset utilization improved over time.

## Skills Demonstrated

- Financial Analysis
- Excel Dashboarding
- Ratio Analysis
- Data Visualization
## Author

Jay Kumar
